...
export default function ProjectPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-10 sm:py-16">
      <Link href="/work" className="inline-flex items-center gap-2 font-body text-sm text-ink-soft hover:text-ink transition-colors mb-10">
        <ArrowLeft size={16} />
        Back to work
      </Link>
      <ProjectVisual id={project.id} bg={bg} textColor={textColor} />
      <div className="space-y-2 mb-8">
        <h1 className="font-display text-4xl sm:text-5xl text-ink leading-tight">{project.title}</h1>
        <p className="font-mono text-sm text-ink-soft/70">{project.category}</p>
      </div>
      <div className="flex flex-wrap gap-3 mb-12">
        {project.link && (
          <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-ink text-cream px-4 py-2 font-body text-sm hover:bg-ink-soft transition-colors">
            <ExternalLink size={14} />
            View live
          </a>
        )}
        {project.instagram && (
          <a href={project.instagram} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-line px-4 py-2 font-body text-sm text-ink hover:bg-cream-warm transition-colors">
            @hueman.story
          </a>
        )}
      </div>
