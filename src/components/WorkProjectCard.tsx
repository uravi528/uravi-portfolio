  ...
  return (
    <Link href={`/projects/${project.id}`}>
      <motion.div
        ...
        className={`group relative flex flex-col justify-between rounded-2xl border p-6 h-60 sm:h-64 cursor-pointer ${s.card}`}
      >
        ...
        <div>
          {project.status === "building" && (
            <span className={`inline-block font-mono text-[11px] uppercase tracking-wide mb-2 ${isDark ? "text-cream/50" : "text-ink-soft/60"}`}>
              in progress
            </span>
          )}
          <h3 className={`font-display text-xl leading-snug mb-1 ${s.text}`}>
            {project.title}
          </h3>
          <p className={`font-body text-sm ${s.sub}`}>
            {project.summary}
          </p>
        </div>
      </motion.div>
